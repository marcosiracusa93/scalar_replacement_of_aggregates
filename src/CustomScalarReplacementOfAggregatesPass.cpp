...
bool check_ptr_expandability(llvm::Use &ptr_use, llvm::Value *base_ptr,
                             std::map<llvm::Use *, bool> &operand_expandability_map,
                             std::map<llvm::Value *, llvm::Value *> &point_to_set_map,
                             bool (*is_relevant_call_fun)(llvm::CallInst *call_inst),
   ...
   if (llvm::GetElementPtrInst *gep_inst = llvm::dyn_cast<llvm::GetElementPtrInst>(ptr_use.getUser())) {
      if (ptr_use.getOperandNo() == gep_inst->getPointerOperandIndex()) {
         ...
         for (llvm::Use &use : gep_inst->uses()) {
            ret = ret and check_ptr_expandability(use, base_ptr, operand_expandability_map, point_to_set_map,
                                                  is_relevant_call_fun, constant_access);
         }
      ...
      return ptr_use.getOperandNo() == store_inst->getPointerOperandIndex();
       ...
       if (!constant_access) {
           ...
           for (llvm::Use &use : ptr_arg->uses()) {
               bool op_exp = check_ptr_expandability(use, base_ptr, operand_expandability_map, point_to_set_map,
                                                     is_relevant_call_fun, true);
               ret = ret and op_exp;
           }
...
void compute_expandability(llvm::Function *kernel_function,
                           llvm::Module *module,
   ...
   for (llvm::GlobalVariable &global_var : module->globals()) {
      for (llvm::Use &use : global_var.uses()) {
         ...
         if (is_in_inner_function) {
            bool can_exp = check_ptr_expandability(use, &global_var, operand_expandability_map, point_to_set_map, is_relevant_call, true);
...
std::vector<unsigned long long> get_op_dims(llvm::Use *op_use, llvm::CallInst *parent_call_isnt,
                                            const std::map<llvm::Use *, std::vector<unsigned long long>> &op_dims_map)
{
...
void compute_op_dims(llvm::CallInst *call_inst, llvm::CallInst *parent_call_inst,
                     ...
                     const std::map<llvm::Use *, bool> &op_expandability_map,
...
void perform_function_versioning(llvm::CallInst *call_inst, llvm::CallInst *parent_call_inst,
                                 std::map<llvm::CallInst *, std::vector<llvm::CallInst *>> &compact_callgraph,
                                 const std::map<llvm::Value *, llvm::Value *> &point_to_set_map,
                                 const std::map<llvm::Use *, bool> &op_expandability_map,
                                 const std::map<llvm::AllocaInst *, bool> &allocas_expandability_map,
                                 ...
                                 bool just_check_versioning = false) {
...
/*
          llvm::errs() << "\n-----------------------\n";
          llvm::errs() << "CallInst: "; call_inst->dump();
          llvm::errs() << "Synthesized as: " << synthesized_function->getName() << " "; synthesized_function->getFunctionType()->dump();
          for(auto& arg : synthesized_function->args()) {
              arg.dump(); llvm::errs() << "    " << (arg_exp_map.at(&arg) ? std::string("") : std::string("NOT ")) << "EXPANDABLE  " << "( "; for (auto d : arg_dims_map.at(&arg)) { llvm::errs() << d << " "; } llvm::errs() << ")\n";
          }
          llvm::errs() << "-----------------------\n";
*/
...
void check_function_versioning(llvm::CallInst *call_inst, llvm::CallInst *parent_call_inst,
                               std::map<llvm::CallInst *, std::vector<llvm::CallInst *>> &compact_callgraph,
                               const std::map<llvm::Value *, llvm::Value *> &point_to_set_map,
                               const std::map<llvm::Use *, bool> &op_expandability_map,
                               const std::map<llvm::AllocaInst *, bool> &allocas_expandability_map,
   ...
   perform_function_versioning(call_inst, parent_call_inst,
                               compact_callgraph,
                               point_to_set_map,
                               op_expandability_map,
                               allocas_expandability_map,
                               globals_expandability_map,
                               op_dims_map,
                               arg_exp_map,
                               arg_dims_map,
                               function_dim_map,
...
void expand_allocas(llvm::CallInst *call_inst, llvm::Function *kernel_function, const std::map<llvm::CallInst *, std::vector<llvm::CallInst *>> &compact_callgraph, const llvm::DataLayout &DL, std::map<llvm::AllocaInst*, std::vector<llvm::AllocaInst*>> &exp_allocas_map) {
...
void expand_signatures_and_call_sites(
         llvm::CallInst *call_inst,
         llvm::CallInst *parent_call_inst,
         std::map<llvm::CallInst *, std::vector<llvm::CallInst *>> &compact_callgraph,
         std::map<llvm::Function *, llvm::Function *> &exp_fun_map,
         std::map<llvm::Use *, bool> &op_expandability_map,
         std::map<llvm::Use *, std::vector<unsigned long long>> &op_dimensions_map,
         ...
         std::set<llvm::Instruction *> &inst_to_remove)
{
   ...
   if (call_it != compact_callgraph.end()) {
      std::vector<llvm::CallInst *> &call_inst_vec = call_it->second;
       ...
       for (llvm::CallInst *inner_call_inst : call_inst_vec) {
           expand_signatures_and_call_sites(inner_call_inst,
                                            call_inst,
                                            compact_callgraph,
                                            exp_fun_map,
                                            op_expandability_map,
                                            op_dimensions_map,
                                            arg_expandability_map,
                                            arg_dimensions_map,
                                            expanded_arguments_map,
                                            inst_to_remove);
       }
   }
   ...
   if (call_inst) {
      ...
      if (called_function) {
         ...
         llvm::Function *expanded_function = nullptr;

         auto fun_it = exp_fun_map.find(called_function);
         if (fun_it == exp_fun_map.end()) {
             ...
             class ExpArgs {
             public:
                 static void rec(ArgObj *arg, std::map<ArgObj *, std::vector<ArgObj *>> &exp_args_map_ref,
             ...
             for (auto arg_it = arg_it_b; arg_it != arg_it_e; arg_it++) {
                 ...
                 llvm::Use *op_arg_use = &call_inst->getArgOperandUse(arg->getArgNo());
...
*/
             ...
             expanded_function = new_function;
         } else {
            expanded_function = fun_it->second;
         }
        ...
        public:
         ...
         llvm::CallInst *new_call_inst = llvm::CallInst::Create(expanded_function, new_call_ops,
         ...
         call_inst->replaceAllUsesWith(new_call_inst);
          ...
          for (llvm::BasicBlock &bb : *expanded_function) {
              for (llvm::Instruction &i : bb) {
                  if (llvm::CallInst *inner_call = llvm::dyn_cast<llvm::CallInst>(&i)) {
                      if (is_relevant_call(inner_call)) {
                          call_inst_vec.push_back(inner_call);
                      }
                  }
              }
          }

          compact_callgraph.insert(std::make_pair(new_call_inst, call_inst_vec));

          std::vector<llvm::CallInst *> &parent_call_inst_vec = compact_callgraph.at(parent_call_inst);

          for (llvm::CallInst *&inner_call_inst : call_inst_vec) {
              if (inner_call_inst == call_inst) {
                  inner_call_inst = new_call_inst;
              }
          }
         ...
         for (unsigned short idx = 0; idx < call_inst->getNumArgOperands(); ++idx) {
            if (call_inst->getArgOperand(idx)->getType()->isPointerTy()) {
               call_inst->setArgOperand(idx, llvm::ConstantPointerNull::get(
                       llvm::dyn_cast<llvm::PointerType>(call_inst->getArgOperand(idx)->getType())));
            }
         }
                                   ...
                                   const llvm::DataLayout &DL)
{
    ...
    while(offset_to_exp > 0)
    {
        ...
        else
        {
            ...
            llvm::errs() << "Offset: " << offset << " (" << offset_to_exp << ")\n";
            llvm::errs() << "ElToExp: "; el_to_exp->dump();
            exit(-1);
        }
    }
...
llvm::Value* get_expanded_value(const std::map<llvm::Argument*, std::vector<llvm::Argument*>>& exp_args_map,
                                ...
                                const std::map<llvm::Argument*, std::vector<unsigned long long>>& arg_size_map,
                                bool is_expansion_allowed,
                                const llvm::DataLayout &DL,
                                llvm::Value* base_address,
                                signed long long offset,
                                unsigned long long accessed_size,
                                llvm::Argument* arg_if_any,
                                llvm::Use* use)
{
    /*
...
void expand_types(T* ptr,
                  ...
                  std::vector<llvm::Type*>& exp_types_ref)
{
            ...
            if (llvm::isa<llvm::StoreInst>(ptr_u->getUser()) or llvm::isa<llvm::LoadInst>(ptr_u->getUser())) {
                ...
                if (is_constant) {
                    ...
                    llvm::Value *exp_val = get_expanded_value(exp_args_map, exp_allocas_map, exp_globals_map,
                                                              arg_size_map, has_expandable_base, DL, base_address, constant_sum,
                                                              accessed_size, nullptr, ptr_u);
                    ...
                    if (wrap_non_const) {
                        ...
                        if (return_type->isVoidTy()) {
                            ...
                            ret = llvm::ReturnInst::Create(user_inst->getContext(), llvm::UndefValue::get(return_type),
                                                           bb);
                        }
                    ...
                    for (llvm::Type *exp_ty : expanded_types) {
                        ...
                        llvm::CmpInst *cond = llvm::CmpInst::Create(llvm::CmpInst::OtherOps::ICmp,
                                                                    llvm::CmpInst::Predicate::ICMP_EQ, offset, bytes_c,
                                                                    cmp_name, split_before);
                        ...
                        llvm::Value *exp_val = get_expanded_value(exp_args_map, exp_allocas_map, exp_globals_map,
                                                                  arg_size_map, has_expandable_base, DL, base_address, bytes_acc - type_size,
                ...
                if (is_constant) {
                    ...
                    if (exp_arg_it != exp_args_map.end()) {
                        ...
                        for (llvm::Argument *exp_arg_u : exp_arg_it->second) {
                            ...
                            llvm::Value *exp_val = get_expanded_value(exp_args_map, exp_allocas_map, exp_globals_map,
                                                                      arg_size_map, has_expandable_base, DL, base_address, arg_offset,
                                                                      accessed_size, arg_u, ptr_u);
                        ...
                        for (unsigned long long op_i = 0; op_i < call_inst->getNumArgOperands(); op_i++) {
                            ...
                            if (arg_expandability_map.at(arg)) {
                                if (op_u->get()->getType()->isPointerTy()) {
                                    process_pointer(op_u, new_bb, inst_to_remove, exp_args_map, exp_allocas_map,
                                                    exp_globals_map, arg_dimensions_map, DL);
                                }
                            }
                        }
                    }
...
class BA_InstChecker
{
public:
    static bool check_ptr(llvm::Value* ptr, std::map<llvm::Value*, bool>& inst_expandability_map, std::set<llvm::Value *> &traversed_ptrs, const std::map<llvm::Argument*, bool> &arg_expandability_map ,bool *constant_access_ptr = nullptr)
    {
       llvm::Value* ptr_rec = ptr;

       auto exp_it = inst_expandability_map.find(ptr_rec);
       if(exp_it != inst_expandability_map.end())
       {
          return exp_it->second;
       }

       auto trav_it = traversed_ptrs.find(ptr_rec);
       if(trav_it != traversed_ptrs.end())
       {
          return true;
       } else {
          traversed_ptrs.insert(ptr_rec);
       }
       ...
       do
       {
          ...
          {
             check_ptr(select_inst->getTrueValue(), inst_expandability_map, traversed_ptrs, arg_expandability_map, constant_access_ptr);
             check_ptr(select_inst->getFalseValue(), inst_expandability_map, traversed_ptrs, arg_expandability_map, constant_access_ptr);
             return false;
          }
          else if(llvm::BitCastOperator* bitcast_op = llvm::dyn_cast<llvm::BitCastOperator>(ptr_rec))
          {
             check_ptr(bitcast_op->getOperand(0), inst_expandability_map, traversed_ptrs, arg_expandability_map, constant_access_ptr);
             return false;
          }
          ...
          {
             return arg_expandability_map.at(arg);
          }
          ...
          else if(llvm::PHINode* phi_node = llvm::dyn_cast<llvm::PHINode>(ptr_rec))
          {
             ...
             {
                check_ptr(*it, inst_expandability_map, traversed_ptrs, arg_expandability_map, constant_access_ptr);
             }
             return false;
          }
          else
    ...
    static bool check_inst(llvm::Instruction* inst, std::map<llvm::Value*, bool>& inst_expandability_map, const std::map<llvm::Argument*, bool> &arg_expandability_map)
    {
       if(llvm::LoadInst* load_inst = llvm::dyn_cast<llvm::LoadInst>(inst))
       {
          std::set<llvm::Value*> traversed_ptrs;
          bool expandable = check_ptr(load_inst->getPointerOperand(), inst_expandability_map, traversed_ptrs, arg_expandability_map);

          inst_expandability_map.insert(std::make_pair(load_inst, expandable));
          for (auto it : traversed_ptrs) {
             inst_expandability_map.insert(std::make_pair(it, expandable));
          }
       ...
       {
          std::set<llvm::Value*> traversed_ptrs;
          bool expandable = check_ptr(store_inst->getPointerOperand(), inst_expandability_map, traversed_ptrs, arg_expandability_map);

          inst_expandability_map.insert(std::make_pair(store_inst, expandable));
          for (auto it : traversed_ptrs) {
             inst_expandability_map.insert(std::make_pair(it, expandable));
          }
       ...
       /*else if(llvm::CallInst* call_inst = llvm::dyn_cast<llvm::CallInst>(inst))
       {
          ...
          for(llvm::Value* op : call_inst->arg_operands())
          {
             if(op->getType()->isPointerTy())
             {
                ...
                if(check_ptr_expandability(op, traversed_instructions_for_op, &constant_access) and constant_access)
                {
...
bool CustomScalarReplacementOfAggregatesPass::runOnModule(llvm::Module& module)
{
   ...
   if(sroa_phase == SROA_functionVersioning)
   {
      ...
      std::map<llvm::Value *, llvm::Value *> point_to_set_map;
       ...
       compute_expandability(kernel_function, &module, allocas_expandability_map,
                             globals_expandability_map, operands_expandability_map,
                             point_to_set_map, compact_callgraph);
   ...
   if(sroa_phase == SROA_disaggregation)
   {
      ...
      check_function_versioning(nullptr, nullptr, compact_callgraph, point_to_set_map, operands_expandability_map, allocas_expandability_map, globals_expandability_map, operands_dimensions_map, arguments_expandability_map, arguments_dimensions_map, function_dim_map);
      ...
      expand_signatures_and_call_sites(nullptr,
                                       nullptr,
                                       compact_callgraph,
                                       exp_fun_map,
                                       operands_expandability_map,
                                       operands_dimensions_map,
                                       arguments_expandability_map,
                                       arguments_dimensions_map,
                                       exp_args_map,
                                       inst_to_remove);
...
/*
       ...
       compute_op_dims_and_perform_function_versioning(kernel_function, inner_functions, false, arg_size_map, arg_expandability_map);
*/
       ...
       for (auto cc_it : compact_callgraph) {
           if (cc_it.first) {
               function_worklist.insert(cc_it.first->getCalledFunction());
           }
       }
...
/*
       ...
       cleanup_(exp_fun_map, inner_functions, inst_to_remove, exp_args_map, exp_globals_map, avoid_expansion);
*/
       assert(!llvm::verifyModule(module, &llvm::errs()));
...
bool CustomScalarReplacementOfAggregatesPass::SROA_wrapperInliningStep(llvm::Function* kernel_function, llvm::Module& module, ModulePass* modulePass)
{
   ...
   compute_op_dims_and_perform_function_versioning(kernel_function, inner_functions, false, arg_size_map, arg_expandability_map);
    ...
    inline_wrappers_(kernel_function, inner_functions, modulePass);
...
void CustomScalarReplacementOfAggregatesPass::expand_ptrs_(llvm::Function *kernel_function,
                                                           std::vector<llvm::Function *> &inner_functions,
                                                           inst_set_ty &inst_to_remove,
                                                           std::map<llvm::Argument *, std::vector<llvm::Argument *>> &exp_args_map,
                                                           std::map<llvm::AllocaInst *, std::vector<llvm::AllocaInst *>> &exp_allocas_map,
...
bool CustomScalarReplacementOfAggregatesPass::check_assumptions(llvm::Function* kernel_function, std::set<llvm::Value*>& avoid_expansion, const std::map<llvm::Argument *, bool> &arg_expandability_map)
{
   ...
   std::map<llvm::Value *, bool> inst_expandability_map;
   ...
   for(unsigned long long idx = 0; idx < call_inst_vec.size(); ++idx)
   {
      ...
      for(llvm::BasicBlock& bb : *called_function)
      {
         for(llvm::Instruction& i : bb)
         {
            ...
            {
               BA_InstChecker::check_inst(&i, inst_expandability_map, arg_expandability_map);
                                                                                              ...
                                                                                              std::map<llvm::Argument*, bool> &arg_expandability_map)
{
   ...
   for(unsigned long long idx = 0; idx < call_inst_vec.size(); ++idx)
   {
      ...
      for(unsigned long long op_idx = 0; op_idx < call_inst->getNumArgOperands(); ++op_idx)
      {
         llvm::Value *arg_op = call_inst->getArgOperandUse(op_idx).get();
         std::set<llvm::Value*> traversed_instructions_for_op;
         bool constant_access = true;
         std::map<llvm::Value *, bool> inst_expandability;
         bool check_ptr = (arg_op->getType()->isPointerTy() ? BA_InstChecker::check_ptr(arg_op, inst_expandability, traversed_instructions_for_op, arg_expandability_map, &constant_access) : false);
      ...
      for(unsigned long long op_idx = 0; op_idx < call_inst->getNumArgOperands(); ++op_idx)
      {
         if (can_exp.at(op_idx)) {
            dimensions.push_back(get_op_dims(&call_inst->getArgOperandUse(op_idx), arg_size_map));
         } else {
            dimensions.push_back(std::vector<unsigned long long>(1, 0));
         }
      }
      ...
      else
      {
         ...
         for(auto& arg : synthesized_function->args())
         {
            ...
            auto exp_it = arg_expandability_map.find(&arg);
            if(exp_it != arg_expandability_map.end())
            {
               bool v1 = exp_it->second;
               bool v2 = can_exp.at(arg.getArgNo());
               if(v1 != v2)
            ...
            {
               arg_expandability_map.insert(std::make_pair(&arg, can_exp.at(arg.getArgNo())));
            }
         }
...
std::vector<unsigned long long> CustomScalarReplacementOfAggregatesPass::get_op_dims(llvm::Use *op_arg_use,
                                                                                     const std::map<llvm::Argument *, std::vector<unsigned long long>> &arg_size_map)
{
...
void CustomScalarReplacementOfAggregatesPass::expand_allocas_(llvm::Function *function,
...
void CustomScalarReplacementOfAggregatesPass::expand_globals_(std::set<llvm::GlobalVariable *> accessed_variables,
                                                              std::map<llvm::GlobalVariable *, std::vector<llvm::GlobalVariable *>> &exp_globals_map,
                                                              std::map<llvm::Argument *, std::vector<unsigned long long>> &arg_size_map,
                                                              const llvm::DataLayout *DL)
{
...
void CustomScalarReplacementOfAggregatesPass::expand_signatures_and_call_sites_(
        ...
        const llvm::DataLayout *DL)
{
   ...
   expand_allocas_(kernel_function, alloca_to_remove, exp_allocas_map, arg_size_map, DL);
   for(llvm::Function* function : inner_functions)
   {
      expand_allocas_(function, alloca_to_remove, exp_allocas_map, arg_size_map, DL);
   }

} // end expand_signatures_and_call_sites_

void CustomScalarReplacementOfAggregatesPass::cleanup_(std::map<llvm::Function *, llvm::Function *> &exp_fun_map,
                                                       std::vector<llvm::Function *> &inner_functions,
                                                       inst_set_ty &inst_to_remove,
                                                       std::map<llvm::Argument *, std::vector<llvm::Argument *>> &exp_args_map,
                                                       std::map<llvm::GlobalVariable *, std::vector<llvm::GlobalVariable *>> &exp_globals_map,
...
void CustomScalarReplacementOfAggregatesPass::inline_wrappers_(llvm::Function *kernel_function,
                                                               std::vector<llvm::Function *> &inner_functions,
