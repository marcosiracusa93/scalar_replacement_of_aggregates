...
public:
    ...
    FunctionDimKey(llvm::Function *const function_ptr, const std::vector<std::vector<unsigned long long>> &arg_dims,
                   const std::vector<bool> &arg_can_exp) : function_ptr(function_ptr), arg_dims(arg_dims),
                                                           arg_can_exp(arg_can_exp) {
    }
    ...
    bool operator<(const FunctionDimKey &oth) const {
        if (function_ptr != oth.function_ptr) {
            ...
            if (arg_can_exp.size() != oth.arg_can_exp.size()) {
                llvm::errs() << "ERR: different number of can_exp arguments (" << arg_can_exp.size() << " vs. "
                             << oth.arg_can_exp.size() << ") for function implementation for \n";
            ...
            if (arg_dims.size() != oth.arg_dims.size()) {
                llvm::errs() << "ERR: different number of dim arguments (" << arg_dims.size() << " vs. "
                             << oth.arg_dims.size() << ") for function implementation for \n";
                llvm::errs() << function_ptr->getName() << " ";
                function_ptr->getFunctionType()->dump();
                llvm::errs() << oth.function_ptr->getName() << " ";
                oth.function_ptr->getFunctionType()->dump();
            ...
            if (called_function->size() > 0) {
                if (called_function->getBasicBlockList().size() != 0) {
                    if (!called_function->isVarArg()) {
                        bool is_wrapper = strncmp(called_function->getName().str().c_str(),
...
bool has_expandable_size(llvm::Value *aggregate,
    ...
    if (aggregate->getType()->isPointerTy()) {
        ...
        if (is_aggregate_type or is_gvar_aggregate or is_array_arg) {
            ...
            llvm::Type *ptr_ty = llvm::isa<llvm::GlobalVariable>(aggregate) ? llvm::dyn_cast<llvm::GlobalVariable>(
                    aggregate)->getValueType() : aggregate->getType()->getPointerElementType();
            if (is_array_arg) {
    ...
    for (llvm::GlobalVariable &global_var : module->globals()) {
        for (llvm::Use &use : global_var.uses()) {
            ...
            if (is_in_inner_function) {
                bool can_exp = check_ptr_expandability(use, &global_var, operand_expandability_map, point_to_set_map,
...
void compute_op_dims(llvm::CallInst *call_inst, llvm::CallInst *parent_call_inst,
    ...
    if (call_it != compact_callgraph.end()) {
        for (llvm::CallInst *inner_call_inst : call_it->second) {
            compute_op_dims(inner_call_inst, call_inst, compact_callgraph, point_to_set_map, op_expandability_map,
                            allocas_expandability_map, globals_expandability_map, op_dims_map);
        }
    }
}
...
void perform_function_versioning(llvm::CallInst *call_inst, llvm::CallInst *parent_call_inst,
    ...
    if (call_it != compact_callgraph.end()) {
        for (llvm::CallInst *inner_call_inst : call_it->second) {
            perform_function_versioning(inner_call_inst, call_inst, compact_callgraph, point_to_set_map,
                                        op_expandability_map, allocas_expandability_map, globals_expandability_map,
                                        op_dims_map, arg_exp_map, arg_dims_map, function_dim_map);
        }
    }
    ...
    if (call_inst) {
        if (llvm::Function *called_function = call_inst->getCalledFunction()) {
            ...
            if (fd_it != function_dim_map.end()) {
                ...
                if (just_check_versioning) {
                 ...
                 for(auto d1 : dimensions)
                 {
                    ...
                    if (used_names.find(Fname) != used_names.end()) {
                        ...
                        while (used_names.find(Fname)->second.find(new_name + s) !=
                               used_names.find(Fname)->second.end()) {
                            ++index;
                        }
                ...
                function_dim_map.insert(
                        std::make_pair(FunctionDimKey(called_function, dimensions, can_exp), synthesized_function));
            }
...
*/
            ...
            if (!just_check_versioning) {
                ...
                if (synthesized_function->getNumUses() > 0) {
                    llvm::CallInst *first_call_use = llvm::dyn_cast<llvm::CallInst>(
                            synthesized_function->use_begin()->getUser());
...
void expand_allocas(llvm::CallInst *call_inst, llvm::Function *kernel_function,
                    const std::map<llvm::CallInst *, std::vector<llvm::CallInst *>> &compact_callgraph,
                    const llvm::DataLayout &DL,
...
void expand_globals(llvm::Module *module, const llvm::DataLayout &DL,
    ...
    for (unsigned long long g_idx = 0; g_idx < globals_to_exp.size(); g_idx++) {
        ...
        if (llvm::StructType *str_ty = llvm::dyn_cast<llvm::StructType>(g_var->getType()->getPointerElementType())) {
            for (unsigned e_idx = 0; e_idx < str_ty->getStructNumElements(); ++e_idx) {
                ...
                llvm::GlobalVariable *new_g_var = new llvm::GlobalVariable(*g_var->getParent(), element,
                                                                           g_var->isConstant(), g_var->getLinkage(),
...
void compute_base_and_offset(llvm::Use *ptr_use, llvm::Value *&base_address, std::vector<llvm::Value *> &offset_chain,
                             std::vector<llvm::Instruction *> &inst_chain, const llvm::DataLayout &DL) {
...
llvm::Value *get_element_at_offset(I *base_address,
        ...
        if (llvm::Argument *base_arg = llvm::dyn_cast<llvm::Argument>(base_address)) {
            ...
            if (exp_it != map.end()) {
                ...
                if (DL.getTypeAllocSize(subelement->getType()->getPointerElementType()) == accessed_size) {
                    ...
                    return get_element_at_offset(subelement, map, new_offset, accessed_size, arg_size_map, DL);
                }
            } else {
                llvm::errs() << "ERR\n";
                exit(-1);
    ...
    if (llvm::StructType *struct_type = llvm::dyn_cast<llvm::StructType>(base_address->getType()->getPointerElementType())) {
        ...
        if (exp_it != map.end()) {
            ...
            I *subelement = subelements.at(idx);
llvm::errs() << "S: " << DL.getTypeAllocSize(subelement->getType()->getPointerElementType()) << "\n";
            if (DL.getTypeAllocSize(subelement->getType()->getPointerElementType()) == accessed_size) {
                return subelement;
            } else {
...
llvm::Value *get_expanded_value(const std::map<llvm::Argument *, std::vector<llvm::Argument *>> &exp_args_map,
    ...
    if (llvm::AllocaInst *alloca_inst = llvm::dyn_cast<llvm::AllocaInst>(base_address)) {
        ...
        return get_element_at_offset(arg, exp_args_map, offset, accessed_size, arg_size_map, DL, true);
        ...
        for (unsigned long long bb_idx = 0; bb_idx < bbs.size(); ++bb_idx) {
            ...
            for (llvm::Instruction &i : *bb) {
                ...
                if (llvm::LoadInst *load_inst = llvm::dyn_cast<llvm::LoadInst>(&i)) {
                    process_pointer(&load_inst->getOperandUse(0), new_bb, inst_to_remove, exp_args_map, exp_allocas_map,
                                    exp_globals_map, arg_dimensions_map, DL);
                } else if (llvm::StoreInst *store_inst = llvm::dyn_cast<llvm::StoreInst>(&i)) {
                    process_pointer(&store_inst->getOperandUse(1), new_bb, inst_to_remove, exp_args_map,
                                    exp_allocas_map, exp_globals_map, arg_dimensions_map, DL);
                        ...
                        for (unsigned long long op_i = 0; op_i < call_inst->getNumArgOperands(); op_i++) {
                            ...
                            if (arg_expandability_map.at(arg)) {
                                if (op_u->get()->getType()->isPointerTy()) {
                                    if (llvm::isa<llvm::ConstantPointerNull>(op_u->get())) {
op_u->get()->dump();
op_u->getUser()->dump();
...
void cleanup(llvm::Module &module,
                    ...
                    if (!ptr_ty->getElementType()->isAggregateType()) {
                        for (auto &use : arg->uses()) {
                            if (llvm::LoadInst *load_inst = llvm::dyn_cast<llvm::LoadInst>(use.getUser())) {
                                ...
                                bool is_wrapper = strncmp(call_inst->getCalledFunction()->getName().str().c_str(),
                                                          std::string(wrapper_function_name).c_str(),
                                                          std::string(wrapper_function_name).size()) == 0;
                                if (is_wrapper) {
                                    return false;
    ...
    for (llvm::Function *function : function_worklist) {
        ...
        if (arg_stored_once != nullptr) {
            ...
            llvm::StoreInst *single_store_inst = llvm::dyn_cast<llvm::StoreInst>(
                    arg_stored_once->use_begin()->getUser());

            for (llvm::BasicBlock &bb : *function) {
                if (llvm::ReturnInst *ret_inst = llvm::dyn_cast<llvm::ReturnInst>(bb.getTerminator())) {
                    llvm::ReturnInst *new_ret_inst = llvm::ReturnInst::Create(ret_inst->getContext(),
                                                                              single_store_inst->getValueOperand(),
                                                                              &bb);
                        ...
                        if (CheckArg::loadedOnly_wrapper(&*fun_arg_it)) {
                            for (auto &use : fun_arg_it->uses()) {
                                if (llvm::LoadInst *load_inst = llvm::dyn_cast<llvm::LoadInst>(use.getUser())) {
                                    ...
                                    arg_to_arg.insert(std::make_pair(std::make_pair(call_inst, use.getOperandNo()),
                                                                     &*new_fun_arg_it));
                                } else {
                                    llvm::errs() << "ERR\n";
                                    exit(-1);
...
void inline_wrappers(llvm::Function *kernel_function, std::set<llvm::Function *> &inner_functions,
    ...
    for (llvm::Function *f : inner_functions) {
        ...
        if (doOpt) {
            ...
            const llvm::TargetTransformInfo &TTI = modulePass->getAnalysis<llvm::TargetTransformInfoWrapperPass>().getTTI(
                    *f);
...
static llvm::cl::opt<uint32_t> CSROAInlineThreshold("csroa-inline-threshold", llvm::cl::Hidden, llvm::cl::init(200),
                                                    llvm::cl::desc(
                                                            "number of maximum statements of the single called function after the inline is applied"));
...
bool CustomScalarReplacementOfAggregatesPass::runOnModule(llvm::Module &module) {
    ...
    if (sroa_phase == SROA_functionVersioning) {
        ...
        compute_op_dims(nullptr, nullptr, compact_callgraph, point_to_set_map, operands_expandability_map,
                        allocas_expandability_map, globals_expandability_map, op_dims_map);
        ...
        perform_function_versioning(nullptr, nullptr, compact_callgraph, point_to_set_map, operands_expandability_map,
                                    allocas_expandability_map, globals_expandability_map, op_dims_map, arg_exp_map,
                                    arg_dims_map, function_dim_map);
    ...
    if (sroa_phase == SROA_disaggregation) {
        ...
        check_function_versioning(nullptr, nullptr, compact_callgraph, point_to_set_map, operands_expandability_map,
                                  allocas_expandability_map, globals_expandability_map, operands_dimensions_map,
                                  arguments_expandability_map, arguments_dimensions_map, function_dim_map);
