...
namespace llvm {
    ...
    llvm::Type *getMemcpyLoopLoweringTypeLocal(llvm::LLVMContext &Context, llvm::ConstantInt *Length, unsigned SrcAlign,
                                               unsigned DestAlign, llvm::Value *SrcAddr, llvm::Value *DstAddr,
                                               const llvm::DataLayout *DL, bool isVolatile, bool &Optimize) {
        if (!isVolatile) {
    ...
    void getMemcpyLoopResidualLoweringTypeLocal(llvm::SmallVectorImpl<llvm::Type *> &OpsOut, llvm::LLVMContext &Context,
                                                unsigned RemainingBytes, unsigned SrcAlign, unsigned DestAlign) {
    ...
    void createMemCpyLoopKnownSizeLocal(llvm::Instruction *InsertBefore, llvm::Value *SrcAddr, llvm::Value *DstAddr,
        ...
        if (llvm::dyn_cast<llvm::BitCastInst>(SrcAddr) &&
            ...
            srcIsGlobal = true;
            do_unrolling = do_unrolling && (dyn_cast<llvm::GlobalVariable>(
                    cast<llvm::ConstantExpr>(SrcAddr)->getOperand(0))->isConstant() ||
        ...
        if (do_unrolling && !SrcIsVolatile && !DstIsVolatile && (srcIsAlloca || srcIsGlobal) &&
            ...
            auto srcAddress = Builder.CreateBitCast(
                    srcIsAlloca ? llvm::dyn_cast<llvm::BitCastInst>(SrcAddr)->getOperand(0) : cast<llvm::ConstantExpr>(
                            SrcAddr)->getOperand(0), SrcOpType);
            auto dstAddress = Builder.CreateBitCast(llvm::dyn_cast<llvm::BitCastInst>(DstAddr)->getOperand(0),
                                                    DstOpType);
            for (auto LI = 0u; LI < LoopEndCount; ++LI) {
                llvm::Value *SrcGEP = Builder.CreateInBoundsGEP(LoopOpType, srcAddress,
                                                                llvm::ConstantInt::get(TypeOfCopyLen, LI));
                llvm::Value *Load = Builder.CreateLoad(SrcGEP, SrcIsVolatile);
                llvm::Value *DstGEP = Builder.CreateInBoundsGEP(LoopOpType, dstAddress,
                                                                llvm::ConstantInt::get(TypeOfCopyLen, LI));
        ...
        if (RemainingBytes) {
            ...
            for (auto OpTy : RemainingOps) {
                ...
                llvm::Value *SrcGEP = RBuilder.CreateInBoundsGEP(OpTy, CastedSrc,
                                                                 llvm::ConstantInt::get(TypeOfCopyLen, GepIndex));
                ...
                llvm::Value *DstGEP = RBuilder.CreateInBoundsGEP(OpTy, CastedDst,
                                                                 llvm::ConstantInt::get(TypeOfCopyLen, GepIndex));
    ...
    void expandMemSetAsLoopLocal(llvm::MemSetInst *Memset, const llvm::DataLayout *DL) {
        ...
        auto ActualCopyLen = AlignCanBeUsed ? llvm::ConstantInt::get(TypeOfCopyLen,
                                                                     cast<llvm::ConstantInt>(CopyLen)->getValue().udiv(
                                                                             llvm::APInt(
                                                                                     TypeOfCopyLen->getIntegerBitWidth(),
                                                                                     Align))) : CopyLen;
